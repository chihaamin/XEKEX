...
CreateMenu.Menu = {}
CreateMenu.actions = {}
  function CreateMenu.actions:new(index,fn)
    if type(index) == 'number' then
    self[index] = fn
    return self
    else error('actions:new: Index MUST be a number!')
    end
  end
  -- The onSelect function that will be called when an item is selected
  function CreateMenu:onSelect(itemIndex)
      -- Execute the selected item's action if it exists
      if self.actions[itemIndex] then
          self.actions[itemIndex]()
          else error('Item Function Not Defined!')
      end
  end
  
  -- Constructor for Menu class
  function CreateMenu.Menu:new(items, selected, message)
      local obj = {}
      setmetatable(obj, self)
      self.__index = self
      obj.items = items
      obj.selected = selected or nil
      obj.message = message or nil
      obj.selectedIndex = nil
      obj.shouldExit = false
      return obj
  end
  
  -- Display the menu and get user selection
  function CreateMenu.Menu:display()
      -- Show the UI button
      gg.showUiButton()
  
      while not self.shouldExit do
          -- Check if the UI button has been clicked
          if gg.isClickedUiButton() then
              -- Show the menu and get user selection
              self.selectedIndex = gg.choice(self.items, self.selected, self.message)
  
              -- If the user cancels the menu, set selectedIndex to nil
              if self.selectedIndex == nil then
                  self.selectedIndex = nil
              else
                  -- Execute the onSelect function with the selected index as an argument
                      CreateMenu:onSelect(self.selectedIndex)
              end
  
              -- Set shouldExit to true to exit the loop
...
local myMenu = MakeMenu().Menu:new({"Option 1", "Option 2", "Option 3"})   |   this will add Menues
